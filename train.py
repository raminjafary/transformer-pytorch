...
from config import get_wieghts_file_path, get_config
...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # precomupte the encoder output and reuse it for every token we get from the deocoder
    encoder_output = model.encode(source, source_mask)
    # Initilize the decoder input with sos token
    ...
    while True:
        ...
        deocder_output = model.decode(
            encoder_output, source_mask, decoder_input, decoder_mask)

        # Get the next token
        probability = model.project(deocder_output[:, -1])

        # select the token with the max probability for it is a greedy search
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            assert encoder_input.size(
                0) == 1, 'Batch size nust be 1 for validation'
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = WordLevelTrainer(
            special_tokens=['[UNK]', '[PAD]', '[SOS]', '[EOF]'], min_frequency=2)
        tokenizer.train_from_iterator(
            get_all_sentences(ds, lang), trainer=trainer)
        tokenizer.sava(str(tokenizer_path))
    else:
...
def get_ds(config):
    ...
    train_ds = BilingualDataset(
        train_ds_raw, tokenizer_src, tokenizer_tgt, config[
            'lang_src'], config['lang_tgt'], config['seq_len']
    )

    val_ds = BilingualDataset(
        val_ds_raw, tokenizer_src, tokenizer_tgt, config[
            'lang_src'], config['lang_tgt'], config['seq_len']
    )
...
def train_model(config):
    ...
    if config['preload']:
        model_filename = get_wieghts_file_path(config, config['preload'])
        print(f'Prelloading model {model_filename}')
        state = torch.load(model_filename)
        initial_epoch = config['epoch'] + 1
        optimizer.load_state_dict(state['optimizer_state-dict'])
    ...
    for epoch in range(initial_epoch, config['epoch']):
        ...
        for batch in batch_iterator:
            ...
            batch_iterator.set_postfix(f'Training Loss: {loss.item():6.3f}')
